...
public class ContaminationUtil {
    ...
    public static float calculateRadiationMod(LivingEntity entity) {
        ...
        float koeff = 10.0F;
        return (float) Math.pow(koeff, -(getConfigEntityRadResistance(entity) + HazmatRegistry
                .getResistance(entity))) * mult;
    }
    ...
    private static void applyRadData(Entity e, float f) {
        ...
        if (e instanceof Player && (((Player) e).capabilities.isCreativeMode || e.isSpectator()))
            return;

        if (e instanceof Player && e.ticksExisted < 200)
            return;
        ...
        if (entity.hasCapability(HbmLivingCapability.EntityHbmPropsProvider.ENT_HBM_PROPS_CAP, null)) {
            HbmLivingCapability.IEntityHbmProps ent = entity
                    .getCapability(HbmLivingCapability.EntityHbmPropsProvider.ENT_HBM_PROPS_CAP, null);
            ent.increaseRads(f);
        }
    }
    ...
    private static void applyRadDirect(Entity entity, float f) {
        ...
        if (entity instanceof Player && (((Player) entity).capabilities.isCreativeMode || ((Player) entity).isSpectator()))
            return;

        if (!(entity instanceof LivingEntity))
            return;

        if (((LivingEntity) entity).isPotionActive(HbmPotion.mutation))
            return;
    ...
    public static void printGeigerData(Player player) {
        ...
        RadiationSavedData data = RadiationSavedData.getData(player.level());
    ...
    public static double getPlayerRads(LivingEntity entity) {
        ...
        if (entity instanceof Player)
            rads = rads + HbmLivingProps.getNeutron((Player) entity) * 20;
        return (double) rads;
    }
    ...
    public static float getRads(Entity e) {
        if (e instanceof IRadiationImmune)
            return 0.0F;
        if (e instanceof LivingEntity)
            return HbmLivingProps.getRadiation((LivingEntity) e);
        return 0.0F;
    }
    ...
    public static void applyAsbestos(Entity e, int i, int dmg, int chance) {
        ...
        if (e instanceof Player && ((Player) e).capabilities.isCreativeMode)
            return;
    ...
    public static void radiate(Level world, double x, double y, double z, double range, float rad3d, float dig3d,
                               float fire3d) {
        radiate(world, x, y, z, range, rad3d, dig3d, fire3d, 0, 0);
    }

    public static void radiate(Level world, double x, double y, double z, double range, float rad3d, float dig3d,
                               float fire3d, float blast3d) {
        radiate(world, x, y, z, range, rad3d, dig3d, fire3d, blast3d, range);
    }

    public static void radiate(Level world, double x, double y, double z, double range, float rad3d, float dig3d,
                               float fire3d, float blast3d, double blastRange) {
        List<Entity> entities = world.getEntitiesWithinAABB(Entity.class, new AABB(x - range, y - range, z - range, x + range, y + range, z + range));
        ...
        for (Entity e : entities) {
            ...
            if (fire3d > 0.025) {
                ...
                if (fireDmg > 0.025) {
                    if (fireDmg > 0.1 && e instanceof Player) {
                        Player p = (Player) e;
    ...
    private static boolean isExplosionExempt(Entity e) {
        ...
        if (e instanceof Ocelot ||
                ...
                (e instanceof Player &&
                        ArmorUtil.checkArmor((Player) e, ModItems.euphemium_helmet, ModItems.euphemium_plate,
                                ModItems.euphemium_legs, ModItems.euphemium_boots))) {
            return true;
        }
    ...
    public static boolean contaminate(LivingEntity entity, HazardType hazard, ContaminationType cont, float amount) {
        ...
        switch (hazard) {
            ...
            case NEUTRON:
                ...
                break;
            case DIGAMMA:
                applyDigammaData(entity, amount);
                break;
        }
