...
public class ExplosionNT extends Explosion {
    ...
    public ExplosionNT(Level world, Entity exploder, double x, double y, double z, float strength) {
        super(world, exploder, x, y, z, strength, false, null);
        this.worldObj = world;
    ...
    private void doNTExplosionA() {
            ...
            for (j = 0; j < this.field_77289_h; ++j) {
                for (k = 0; k < this.field_77289_h; ++k) {
                    if (i == 0 || i == this.field_77289_h - 1 || j == 0 || j == this.field_77289_h - 1 || k == 0 || k == this.field_77289_h - 1) {
                        ...
                        d1 /= d3;
                        d2 /= d3;
                        float f1 = this.explosionSize * (0.7F + this.worldObj.random.nextFloat() * 0.6F);
                        d5 = this.explosionX;
                        d6 = this.explosionY;
                        d7 = this.explosionZ;

                        for (float f2 = 0.3F; f1 > 0.0F; f1 -= f2 * 0.75F) {
                            int j1 = (int) Math.floor(d5);
                            int k1 = (int) Math.floor(d6);
                            int l1 = (int) Math.floor(d7);
                            BlockPos pos = new BlockPos(j1, k1, l1);
                            BlockState block = this.worldObj.getBlockState(pos);

                            if (block.getBlock() != AIR) {
                                float f3 = this.exploder != null ? this.exploder
                                        .getBlockExplosionResistance(this, this.worldObj,
                                                new BlockPos(j1, k1, l1), block) : block.getBlock()
                                        .getExplosionResistance(worldObj, new BlockPos(j1, k1, l1),
                                                (Entity) null, this);
                                f1 -= (f3 + 0.3F) * f2;
                            }

                            if (f1 > 0.0F && (this.exploder == null || this.exploder
                                    .canExplosionDestroyBlock(this, this.worldObj, new BlockPos(j1, k1, l1), block, f1))) {
        ...
        if (!has(ExAttrib.NOHURT)) {
            ...
            i = (int) Math.floor(this.explosionX - (double) this.explosionSize - 1.0D);
            j = (int) Math.floor(this.explosionX + (double) this.explosionSize + 1.0D);
            k = (int) Math.floor(this.explosionY - (double) this.explosionSize - 1.0D);
            int i2 = (int) Math.floor(this.explosionY + (double) this.explosionSize + 1.0D);
            int l = (int) Math.floor(this.explosionZ - (double) this.explosionSize - 1.0D);
            int j2 = (int) Math.floor(this.explosionZ + (double) this.explosionSize + 1.0D);
            List list = this.worldObj.getEntities(this.exploder, new AABB((double) i,
            ...
            for (int i1 = 0; i1 < list.size(); ++i1) {
                ...
                if (d4 <= 1.0D) {
                    d5 = entity.position().x - this.explosionX;
                    d6 = entity.position().y + (double) entity.getEyeHeight() - this.explosionY;
                    d7 = entity.position().z - this.explosionZ;
                    ...
                    if (d9 != 0.0D) {
                        ...
                        d6 /= d9;
                        d7 /= d9;
                        double d10 = (double) this.worldObj.getBlockDensity(new Vec3d(vec3.xCoord,
                                vec3.yCoord, vec3.zCoord), entity.getBoundingBox());
                        double d11 = (1.0D - d4) * d10;
                        float d111 = (float) ((int) ((d11 * d11 + d11) / 2.0D * 8.0D * (double) this.explosionSize + 1.0D));
                        entity.hurt(DamageSource.causeExplosionDamage(this), d111);
                        double d8 = d11;
                        ...
                        if (entity instanceof Player) {
                            this.affectedEntities.put((Player) entity, Vec3.createVectorHelper(d5 * d11,
                                    d6 * d11, d7 * d11));
                        }
                    }
                }
    ...
    private void doNTExplosionB(boolean p_77279_1_) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!has(ExAttrib.NOPARTICLE)) {
                double d0 = (float) i + this.worldObj.random.nextFloat();
                double d1 = (float) j + this.worldObj.random.nextFloat();
                double d2 = (float) k + this.worldObj.random.nextFloat();
            ...
            if (block.getBlock() != AIR) {
                if (block.getBlock().canDropFromExplosion(this) && !has(ExAttrib.NODROP)) {
                    ...
                    block.getBlock().dropBlockAsItemWithChance(this.worldObj, chunkposition,
                            this.worldObj.getBlockState(chunkposition), chance, 0);
                }
        ...
        if (has(ExAttrib.FIRE) || has(ExAttrib.BALEFIRE) || has(ExAttrib.LAVA)) {
            ...
            while (iterator.hasNext()) {
                ...
                k = chunkposition.getZ();
                block = this.worldObj.getBlockState(chunkposition);
                BlockState block1 = this.worldObj.getBlockState(new BlockPos(i, j - 1, k));
