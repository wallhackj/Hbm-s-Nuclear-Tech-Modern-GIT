...
public class FluidTypeHandler {
    ...
    public static FluidProperties getProperties(Fluid f) {
        if (f == null)
            return NONE;
        ResourceLocation fluidName = ForgeRegistries.FLUIDS.getKey(f);
        if (fluidName == null)
            return NONE;
        return fluidProperties.getOrDefault(fluidName, NONE);
    }
    ...
    public static boolean isHot(Fluid f) {
        if (f == null)
            return false;
        return f.getFluidType().getTemperature() >= 373;
    }
    ...
    public static boolean containsTrait(Fluid f, FluidTrait t) {
        ...
        ResourceLocation fluidKey = ForgeRegistries.FLUIDS.getKey(f);
        if (fluidKey == null) {
            return false; // Fluid is not registered
        }
        String fluidName = fluidKey.getPath(); // Get the name part of the ResourceLocation
        FluidProperties p = fluidProperties.get(fluidName);
        if (p == null) {
            return false;
        }
